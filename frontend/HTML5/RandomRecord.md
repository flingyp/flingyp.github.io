...
2. 优化搜索引擎：语义化标签的 DOM 结构较为容易的被搜索引擎检索解析到
...
2. **尽量多使用链接**。增加内部链接，使页面之间互相连接，同时也可以提高页面权重；如果条件允许，可以增加外部链接，这对于提高页面权重和曝光度也非常有益。

3. **标题和描述**。对每篇文章进行描述时，应该针对每个页面设置独立的标题和描述，能将网站内容用简要的语言呈现出来，同时也增加了网站在搜索引擎中的曝光机会。

4. **标签语义化**。尽量保证标签的语义化，这样有利于搜索引擎的检索，同时也有利于用户的阅读。

5. **服务端渲染**。考虑服务端渲染，这样可以提高网站的访问速度，同时也有利于搜索引擎的检索。
