...
- ECMAScript：JavaScript 语言基础，提供核心语言功能
...
:::tip
什么是闭包：一个函数和其周围状态的引用捆绑在一起，这样的组合就是闭包。也就是说，闭包让你可以在一个外层函数中访问到其内层函数的作用域。最典型的就是嵌套函数。

我们可以在外部函数中访问内部函数作用域的变量就可以采用闭包的形式
:::
...
9. 新增 Set、Map、Symbol、Proxy、Reflect 等数据类型
10. 新增 Generator 函数
11. 新增 async/await 异步编程

## 46. 浏览器的缓存策略

强缓存（本地缓存）、弱缓存（协商缓存）

本地缓存：不发起请求，直接使用缓存里的内容，浏览器吧 JS、CSS、Image 等资源存到内存中，下次用户再次访问直接从内存中取，提高性能

强缓存的触发：

1. HTTP1.0：时间戳响应标头
2. HTTP1.1：Cache-Control 相应标头

协商缓存：需要向后台发送请求，通过判断来决定是否使用协商缓存，如果请求内容没有发生变化，则返回 304，浏览器就用缓存里的内容，如果发生变化，则返回最新的内容

弱缓存的触发：

1. HTTP1.0：请求头中的 If-Modified-Since 响应头中的 Last-Modified
2. HTTP1.1：请求头中的 If-None-Match 响应头中的 ETag
