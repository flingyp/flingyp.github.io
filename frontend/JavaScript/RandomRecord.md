...
也就是说，闭包让你可以在外层函数中访问到其内层函数的**作用域**。最典型的就是嵌套函数

我们可以在**外部函数中访问内部函数作用域的变量、函数和其他值**就可以采取闭包的形式
...
- 可以在函数体外部访问内部函数的私有变量
- 不需要将啥杂七杂八的变量都定义在全局环境当中。避免变量污染全局
- 把变量存到独立的作用域，作为私有成员的存在

缺点：

- 内部作用域中所定义的变量未及时销毁时会造成内存泄漏，内存泄露积累多了就容易导致内存溢出，导致页面卡顿
...
:::details Async 和 Defer 的区别

- Async（异步）：异步加载，加载完 JS 文件后会立即执行（无法保证多个 JS 文件的执行顺序）
- Defer（延迟）：延迟加载，会和 HTML 的解析同步进行加载 JS，等待 HTML 解析完成后执行 JS 文件 （可以保证多个 JS 文件的执行顺序）
...
不同点：含义不同。Null 是表示的是一个空对象指针，Undefined 一个变量自然的、最原始的状态值
