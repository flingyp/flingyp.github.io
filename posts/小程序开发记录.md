...
<template>
  <scroll-view scroll-x="true" class="scroll-X">
    <block v-for="(item, index) in Data" :key="index">
      <view class="scroll-con" @tap="goDetail(item.id)"> // 滚动的相关内容 </view>
    </block>
  </scroll-view>
</template>
```
...
export default {
  methods: {
    getCode() {
      ...
      // 需要公众号的appid:
      let newhref =
        'https://open.weixin.qq.com/connect/oauth2/authorize?appid=xxxx&redirect_uri=' +
 ...
 * 保存生成的图片到本地相册中
 */
export const posterToPhotosAlbum = (filePath) => {
...
const { drawCanvasImage, drawCircularAvatar, drawText } = this.$refs.savePoster.$options.methods;

// context 为组件画布实例， comThis为组件this
this.$refs.savePoster.canvasInit(({ context, comThis }) => {
  // 获取画布宽高
  const {canvasWH} = comThis;

  // 1. 绘制海报背景图
  const promise_1 = drawCanvasImage(context, this.canvasBgUrl, canvasWH.canvasWidth, canvasWH.canvasHeight);

  // 必须先绘制玩海报背景图 再去操作其他绘制内容，防止背景图覆盖其它绘制的内容。
  promise_1.then((res) => {
    // 2. 绘制用户头像
    const promise_2 = drawCircularAvatar(
      context,
      this.avatarUrl,
    ...
    // 4. 绘制二维码
    const promise_4 = drawCanvasImage(
      context,
      this.qrCodeUrl,
      100,
      100,
  ...
  <view>
    <scroll-view scroll-x="true" class="savePoster" :scroll-with-animation="true" @scroll="scroll">
      <view class="savePosterItem">
        ...
        <save-poster-com v-show="!tempFilePath" ref="savePoster" :canvas-info="canvasInfo"></save-poster-com>
      </view>
...
export default {
  ...
  onLoad() {
    let { drawCanvasImage, drawCircularAvatar, drawText } = this.$refs.savePoster.$options.methods;
    this.$refs.savePoster.canvasInit(({ context, comThis }) => {
      // 获取画布宽高
...
```vue
<template>
  <view class="signature-body" :style="{ width: canvasWidth + '%', height: canvasHeight + 'rpx' }">
    <canvas
      :id="cid"
    ...
    <view class="headerBox">
      <view class="ic-HeaderBox">
        <block v-for="(item, index) in headerList" :key="index">
          <view :class="['headerItem', item.active ? 'headerItem-active' : '']" @click="tapAcitve(index)">{{
            item.name
          }}</view>
        </block>
      </view>
...
```vue
<template>
  <progressBar :progress-title="'进度条标题'" :progress-total-num="1000" :progress-active-num="780"></progressBar>
</template>
```
...
<template>
  <view class="progressBarBox">
    ...
    <view class="barBox" :style="{ height: progressBarHeight + 'rpx' }">
      <view ref="progressBarActive" class="progressBarActive" :style="barActiveStyle"></view>
      <view ref="progressBarTotal" class="progressBarTotal">
...
[支付文档](https://developers.weixin.qq.com/miniprogram/dev/platform-capabilities/business-capabilities/ministore/wxafunds/Introduction.html)

前提条件：小程序需要在后台微信支付功能申请或关联商户号，才可以在微信小程序接入微信支付功能

- 对于前端而言，只需要调用微信小程序提供的支付接口 `wx.requestOrderPayment `，传入支付参数，即可实现支付功能
- 对于后端而言，需要调用微信提供的接口，将支付参数发送给前端，微信会返回支付结果，后端根据支付结果，决定是否将订单状态修改为已支付

### 提现功能

[提现文档](https://developers.weixin.qq.com/miniprogram/dev/platform-capabilities/business-capabilities/ministore/wxafunds/API/funds/submitwithdraw.html)

只需要后端提供接口，去调用微信提供的接口，传入需要的参数即可发起提现

## 微信小程序场景值

[小程序场景值文档](https://developers.weixin.qq.com/miniprogram/dev/framework/app-service/scene.html)

开发者可以在 `onLaunch` 和 `onShow` 的参数 `options` 或者使用 `uni.getLaunchOptionsSync` 中获取场景值

- 可以获取到用户进入的页面 `path` 以及参数 `query`

服务端，通过调用微信提供的接口来获取小程序二维码，[获取小程序码文档](https://developers.weixin.qq.com/miniprogram/dev/OpenApiDoc/qrcode-link/qr-code/getUnlimitedQRCode.html)
