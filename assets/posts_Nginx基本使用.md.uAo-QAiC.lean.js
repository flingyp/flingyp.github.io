import{_ as s,c as a,a3 as e,o as p}from"./chunks/framework.DfGUnYSz.js";const g=JSON.parse('{"title":"Nginx 基本使用","description":"","frontmatter":{},"headers":[],"relativePath":"posts/Nginx基本使用.md","filePath":"posts/Nginx基本使用.md","lastUpdated":1739432323000}'),l={name:"posts/Nginx基本使用.md"};function i(t,n,c,o,r,d){return p(),a("div",null,n[0]||(n[0]=[e(`<h1 id="nginx-基本使用" tabindex="-1">Nginx 基本使用 <a class="header-anchor" href="#nginx-基本使用" aria-label="Permalink to &quot;Nginx 基本使用&quot;">​</a></h1><h2 id="安装" tabindex="-1">安装 <a class="header-anchor" href="#安装" aria-label="Permalink to &quot;安装&quot;">​</a></h2><ol><li>基于 Yum 的 方式安装 Nginx</li></ol><p>先来查看 yum 是否存在 <code>yum list | grep nginx</code> 如果存在并且系统的 nginx 源 也存在，但是版本有些低。我们可以自己配置 yum 源</p><div class="language-nginx vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">nginx</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">[nginx]</span></span>
...
<span class="line"><span>│   └── ...</span></span>
<span class="line"><span>└── ...</span></span></code></pre></div><p>子配置文件 大概 结构 <code>default.conf</code></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>server {</span></span>
...
<span class="line"><span>}</span></span>
<span class="line"><span>监听 8001端口 会自动访问 /usr/share/nginx/html/html8001 下的 index.html</span></span></code></pre></div><h3 id="基于-ip-的虚拟主机" tabindex="-1">基于 IP 的虚拟主机 <a class="header-anchor" href="#基于-ip-的虚拟主机" aria-label="Permalink to &quot;基于 IP 的虚拟主机&quot;">​</a></h3><ul><li>基于 IP 和基于端口的配置几乎一样，只是把<code>server_name</code>选项，配置成 IP 就可以了</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>比如上面的配置 可以修改为</span></span>
<span class="line"><span>server{</span></span>
<span class="line"><span>    listen 80;</span></span>
<span class="line"><span>    server_name xxx.xx.xxx.xxx;</span></span>
<span class="line"><span>    root /usr/share/nginx/html/html8001;</span></span>
<span class="line"><span>    index index.html;</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="基于-nginx-使用-域名设置虚拟主机-常用" tabindex="-1">基于 Nginx 使用 域名设置虚拟主机 （常用） <a class="header-anchor" href="#基于-nginx-使用-域名设置虚拟主机-常用" aria-label="Permalink to &quot;基于 Nginx 使用 域名设置虚拟主机 （常用）&quot;">​</a></h3><p>首先必须先做<strong>域名解析</strong>，才能去 服务器上进行配置。</p><p>那 上面 <strong>基于端口号设置虚拟主机</strong> 的例子来说明：</p><p>如果是修改 <code>nginx</code> 下面的 <code>/etcnginx/conf.d/default.conf</code>文件， 它默认的 <strong>监听端口就是 80</strong>， 所以我们只需要修改一处配置就行。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>server {</span></span>
<span class="line"><span>    listen       80;</span></span>
<span class="line"><span>    server_name  xxx.xxx.com/cn;  // 修改这处配置 为你配置的域名</span></span>
<span class="line"><span>    ... 其他配置</span></span>
<span class="line"><span> }</span></span></code></pre></div><p>如果修改的是 我们自己新建的 <code>xxx.conf</code>文件。 例如 <strong>基于端口号设置虚拟主机</strong> 时的例子 <code>8001nginx.conf 文件</code> 的一些配置信息</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>例子： 再 conf.d 文件夹下 新建了一个为 8001nginx.conf 文件， 在文件配置一些基本信息 如下：</span></span>
<span class="line"><span></span></span>
<span class="line"><span>server{</span></span>
...
<span class="line"><span>    index index.html;</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span>监听 8001端口 会自动访问 /usr/share/nginx/html/html8001 下的 index.html</span></span></code></pre></div><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>注意：</span></span>
<span class="line"><span>	上面两个 可以同时存在的， 虽然监听的端口一样 但是 对应着不同的域名 和 root 服务默认启动目录 不一样</span></span></code></pre></div>`,79)]))}const x=s(l,[["render",i]]);export{g as __pageData,x as default};
